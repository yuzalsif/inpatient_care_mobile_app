...
import 'package:settings/src/log_service.dart';
    ...
    static Future<void> sendBackgroundSMS(String phoneNumber, String messageContent) async {
        ...
        if (await _requestSmsPermission()) {
            ...
            if (result == SmsStatus.sent) {
                ...
                LogService.addLog("SMS sent successfully to $phoneNumber");
            } else {
                print("Failed to send SMS to $phoneNumber");
                LogService.addLog("Failed to send SMS to $phoneNumber");
            }
        } else {
            print("SMS permission not granted");
            LogService.addLog("SMS permission not granted");
        }
    }

    // Fetch data and send SMS for each message dynamically based on the user-entered URL
    static Future<SmsResponse> fetchSmsData(String urlString) async {
        ...
        if (response.statusCode == 200) {
            ...
            LogService.addLog('Failed to load SMS data: ${response.statusCode}');
